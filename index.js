...
const SubRefConstraintError = require('../shared/errors');
...
function getUpdateQueryObjectFor(modelRef, pathRef, modelSubRef, pathSubRef, referencedValues) {
  ...
  result.push({
    [operator]: {
      [updatePath]: operator === '$pull' ? referencedValues : null,
    },
  });
  ...
  if (lastDocumentArray !== null)
    result.push({
      arrayFilters: [{ [`j.${arrayFilterConditionPath}`]: referencedValues }],
    });
...
async function onDeleteCascade(
  ...
  else await mongoose.model(modelRef).remove(queryObject);
}
...
function plugin(modelName, schema) {
  ...
  function setValidator(path, schemaType) {
    ...
    const subRefModel = schemaType.options.subRef.split('.')[0];
    ...
    mongoose
      ...
      .validate(async function (newValues) {
        ...
        if (deletedValues.length > 0) {
          try {
            ...
            // This is the only case in which, this can be used directly (because it can only throw an error)
            if (schemaType.required && !schemaType.required) await remover();
            // Otherwise we will need to run updates, after validation is completed
            else this.$locals.afterValidation = [...(this.$locals.afterValidation || []), remover];
          } catch (e) {
  ...
  function eachPath(path, schemaType) {
    ...
    if (schemaType.constructor.name === 'SchemaArray' && schemaType.options.type[0].subRef) {
      refs[schemaType.options.type[0].subRef] = [
