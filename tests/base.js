...
describe('Sub References - BoundTo - Deleting the parent of the relationship', async function () {
  const PersonSchema = new mongoose.Schema({
    name: {
      type: String,
    },
  ...
  const PersonModel = consistentModel('Person_5', PersonSchema);
  ...
  const MessageModel = consistentModel('Message_5', MessageSchema);
  ...
  it('subRef is required, deleteOne cascade ---> should delete the parent of the relationship and his children', async function () {
    ...
    const children = [
      await new MessageModel({ person: parent, contact: parent.contacts[0]._id }).save(),
      await new MessageModel({ person: parent, contact: parent.contacts[1]._id }).save(),
      await new MessageModel({ person: parent, contact: parent.contacts[0]._id }).save(),
    ];
...
describe('Sub References - BoundTo - Deleting subDocument from the parent of the relationship', async function () {
  const PersonSchema = new mongoose.Schema({
    name: {
      type: String,
    },
  ...
  const PersonModel = consistentModel('Person_6', PersonSchema);
  ...
  const MessageModel = consistentModel('Message_6', MessageSchema);
